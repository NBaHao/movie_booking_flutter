...
class _MoviesState extends State<MoviesScreen>
  ...
  late final _tabController = TabController(
      length: 2,
      vsync: this,
      initialIndex: (context.read<NavigationBloc>().state
              is NavigationMoviePageState
          ? (context.read<NavigationBloc>().state as NavigationMoviePageState)
              .currentMoviePage
              .index
          : 0));
      ...
      child: BlocListener<NavigationBloc, NavigationState>(
        listener: (context, state) {
          if (state is NavigationMoviePageState) {
            _tabController.animateTo(state.currentMoviePage.index);
            _localizations = localizations(context);
            _dataLoadBloc.add(SetLanguageEvent(_localizations.localeName));
          }
        },
                      ...
                      tabs: <Widget>[
                        Tab(
                          ...
                          children: [
                            Expanded(
                                child: Center(
                              child: Text(_localizations.nowplaying,
                        ...
                        Tab(
                          ...
                          children: [
                            Expanded(
                                child: Center(
                                    child: Center(
                              child: Text(_localizations.comingsoon,
                                  style: const TextStyle(
                                      fontSize: 16,
                                      fontWeight: FontWeight.bold)),
                            ))),
